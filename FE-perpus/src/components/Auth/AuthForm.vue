...
const router = useRouter(); // Menggunakan router di sini
...
const handleAuth = async () => {
  if (props.showUsername && userInput.password_confirmation) { 
    ...
    if (!authStore.isError) {
      router.push('/'); // Arahkan ke halaman beranda setelah registrasi
    }
  } else {
    ...
    if (!authStore.isError) {
      router.push('/'); // Arahkan ke halaman beranda setelah login
    }
  }
};
